...
// GET /api/catastro/inmueble?id=<inmueble_id>
export async function GET(req: NextRequest) {
  ...
  const { data: inm, error } = await supabase
    .from('inmuebles')
    .select('id, referencia_catastral, provincia, ciudad')
    .eq('id', id)
    .single()

  if (error || !inm) return NextResponse.json({ error: 'No encontrado' }, { status: 404 })
  if (!inm.referencia_catastral) return NextResponse.json({ error: 'Sin referencia catastral' }, { status: 400 })

  const prov = inm.provincia || ''
  const mun = inm.ciudad || ''
  const catastro = await fetchCatastro(inm.referencia_catastral, prov, mun)
