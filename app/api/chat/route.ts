  ...
  // ── Edificios ────────────────────────────────────────────────────────────────
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        num_plantas: { type: 'number', description: 'Número de plantas del edificio. Buscá expresiones como "PB+3", "4 plantas", "planta baja más tres alturas", "3 pisos", etc. PB+3 = 4 plantas, PB+2 = 3 plantas.' },
        ...
        unidades: {
          ...
          items: {
            ...
            properties: {
              ...
              superficie: { type: 'number', description: 'Superficie en m²' },
              ocupacion: { type: 'string', enum: ['libre', 'alquilado'], description: 'Estado de ocupación' },
  ...
  try {
    ...
    if (name === 'insert_edificio_unidades') {
      ...
      const rows = unidades.map((u: any) => ({
        ...
        ocupacion: (u.ocupacion || 'libre').toLowerCase().replace('arrendado', 'alquilado'),
    ...
    if (htmlFileList.length > 0) {
      ...
      if (scraped.precio || scraped.direccion) {
        ...
        portalCtx = scraped.superficie
          ? `\n\n[${portalName.toUpperCase()} — ARCHIVO ADJUNTO "${first.name}"] Datos extraídos del archivo HTML que subió el usuario (la página no se pudo scrapear por link):\n${campos}\n\nINSTRUCCIÓN: NO llames a analizar_inmueble todavía. Primero mostrale al usuario un resumen breve de estos datos (precio, dirección/ciudad, superficie) y preguntale cuánto estima de reforma para este inmueble — es el dato que más cambia el ROI y no lo tenés. Si responde con un monto, usalo. Si responde "no sé"/"no tengo idea"/similar, usá reforma=0 pero decilo explícitamente al mostrar el resultado ("ROI asumiendo 0€ de reforma, sin confirmar — ajustalo cuando lo sepas"). Recién con esa respuesta llamá a analizar_inmueble con nombre='${scraped.direccion || scraped.titulo || 'Inmueble'}', ciudad='${scraped.ciudad || ''}', precio_compra=${scraped.precio || 0}, metros=${scraped.superficie}${scraped.habitaciones ? `, habitaciones=${scraped.habitaciones}` : ''}, reforma=<lo que haya indicado>.`
          : `\n\n[${portalName.toUpperCase()} — ARCHIVO ADJUNTO "${first.name}"] Datos extraídos del archivo HTML:\n${campos}\n\nFalta la superficie (m²) para calcular comparables y ROI. Pedísela al usuario junto con la reforma estimada; en cuanto tengas ambas, llamá a analizar_inmueble con la reforma indicada (o 0 si dice que no sabe, aclarándolo en la respuesta). Si prefiere no dar la superficie, usá insert_radar con estos datos y fuente='${portalName}'.`
      } else {
    ...
    if (!portalCtx && anyUrlMatch) {
      ...
      if (portalName) {
        ...
        if ('error' in scraped) {
          ...
          portalCtx = scraped.superficie
            ? `\n\n[${portalName.toUpperCase()} DETECTADO] Datos extraídos del link:\n${campos}\nURL: ${rawUrl}\n\nINSTRUCCIÓN: NO llames a analizar_inmueble todavía. Primero mostrale al usuario un resumen breve de estos datos (precio, dirección/ciudad, superficie) y preguntale cuánto estima de reforma para este inmueble — es el dato que más cambia el ROI y no lo tenés. Si responde con un monto, usalo. Si responde "no sé"/"no tengo idea"/similar, usá reforma=0 pero decilo explícitamente al mostrar el resultado ("ROI asumiendo 0€ de reforma, sin confirmar — ajustalo cuando lo sepas"). Recién con esa respuesta llamá a analizar_inmueble con nombre='${scraped.direccion || scraped.titulo || 'Inmueble'}', ciudad='${scraped.ciudad || ''}', precio_compra=${scraped.precio || 0}, metros=${scraped.superficie}${scraped.habitaciones ? `, habitaciones=${scraped.habitaciones}` : ''}, reforma=<lo que haya indicado>, url='${rawUrl}'.`
            : `\n\n[${portalName.toUpperCase()} DETECTADO] Datos extraídos del link:\n${campos}\nURL: ${rawUrl}\n\nFalta la superficie (m²) para calcular comparables y ROI. Pedísela al usuario junto con la reforma estimada; en cuanto tengas ambas, llamá a analizar_inmueble con la reforma indicada (o 0 si dice que no sabe, aclarándolo en la respuesta) y url='${rawUrl}'. Si prefiere no dar la superficie, usá insert_radar con estos datos, fuente='${portalName}' y url='${rawUrl}'.`
        }
      }
    }
