...
type Movimiento = {
  ...
  tipo: string; categoria: string; proyecto_id: string; cuenta: string | null
  proyectos?: { nombre: string; porcentaje_hasu: number }
}

const CATEGORIAS = ['Materiales','Mano de obra','Honorarios','Impuestos','Venta','Arras','Compra','Reforma','Transferencia','Aportación','Otros']
...
export default function FlujoCajaPage() {
  ...
  // Cuentas únicas presentes en los movimientos
  ...
  // Movimientos filtrados por cuenta y proyecto
  const filtered = useMemo(() => movimientos.filter(m => {
    if (filtroCuenta   !== 'todas' && m.cuenta      !== filtroCuenta)   return false
    if (filtroProyecto !== 'todos' && m.proyecto_id !== filtroProyecto) return false
    return true
  }), [movimientos, filtroCuenta, filtroProyecto])

  // ── Cálculo correcto HASU ──────────────────────────────────────
  // 1. Agrupar por proyecto
  // 2. Por proyecto: ingresos_op (excl. capital) y gastos reales
  // 3. beneficio_proyecto = ingresos_op - gastos
  // 4. Aplicar porcentaje_hasu SOLO sobre el beneficio
  // 5. Las tarjetas Ingresos/Gastos muestran totales reales; Balance = beneficio × %
  const totals = useMemo(() => {
    const byProject: Record<string, { ingresos: number; gastos: number; pct: number }> = {}

    filtered.forEach(m => {
      const pid = m.proyecto_id || '__sin__'
    ...
    let ingresos = 0, gastos = 0, balanceHasu = 0
    Object.values(byProject).forEach(({ ingresos: ing, gastos: gas, pct }) => {
      ingresos     += ing
      gastos       += gas
      balanceHasu  += (ing - gas) * (pct / 100)
    })
    return { ingresos, gastos, balanceHasu }
  }, [filtered])

  // ── Porcentaje HASU del proyecto seleccionado (para label) ────
  const pctLabel = useMemo(() => {
    if (filtroProyecto === 'todos') return null
    const m = movimientos.find(m => m.proyecto_id === filtroProyecto)
    const pct = (m?.proyectos as any)?.porcentaje_hasu
    return pct != null && pct < 100 ? `JV ${pct}%` : null
  }, [filtroProyecto, movimientos])
    ...
    <div className="p-4" style={{ background: '#0A0A0A', minHeight: '100vh' }}>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={() => { setEditingId(null); setForm(emptyForm()); setShowForm(true) }}
          ...
          style={{ background: '#F26E1F' }}>+ Movimiento</button>
      </div>
      ...
      <div className="flex flex-col gap-2 mb-4">
        <div className="flex gap-2">
          ...
          <select value={filtroCuenta} onChange={e => setFiltroCuenta(e.target.value)}
            className="flex-1 text-sm font-bold rounded-xl px-3 py-2 outline-none"
            style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.08)', color: filtroCuenta !== 'todas' ? '#F26E1F' : '#ccc' }}>
          ...
          <select value={filtroProyecto} onChange={e => setFiltroProyecto(e.target.value)}
            className="flex-1 text-sm font-bold rounded-xl px-3 py-2 outline-none"
            style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.08)', color: filtroProyecto !== 'todos' ? '#F26E1F' : '#ccc' }}>
            <option value="todos">Todos los proyectos</option>
            {proyectos.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
          </select>
        </div>
        {/* Reset */}
        {(filtroCuenta !== 'todas' || filtroProyecto !== 'todos') && (
          <button onClick={() => { setFiltroCuenta('todas'); setFiltroProyecto('todos') }}
            className="text-xs font-bold self-end px-3 py-1 rounded-lg"
            style={{ color: '#F26E1F', background: 'rgba(242,110,31,0.1)' }}>
            ✕ Limpiar filtros
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="rounded-xl p-3" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div className="font-black text-[14px] leading-none" style={{ color: '#22C55E' }}>{fmt(totals.ingresos)}</div>
            <div className="text-[9px] mt-1 font-medium" style={{ color: '#555' }}>operativos</div>
          </div>
          <div className="rounded-xl p-3" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div className="text-[10px] font-bold uppercase tracking-wide mb-1" style={{ color: '#888' }}>Gastos</div>
            <div className="font-black text-[14px] leading-none" style={{ color: '#EF4444' }}>{fmt(totals.gastos)}</div>
            <div className="text-[9px] mt-1 font-medium" style={{ color: '#555' }}>totales</div>
          </div>
          <div className="rounded-xl p-3" style={{ background: '#1a1000', border: '1px solid rgba(242,110,31,0.3)' }}>
            <div className="text-[10px] font-bold uppercase tracking-wide mb-1" style={{ color: '#F26E1F' }}>
              Balance HASU{pctLabel ? ` · ${pctLabel}` : ''}
            </div>
            <div className="font-black text-[14px] leading-none" style={{ color: totals.balanceHasu >= 0 ? '#F26E1F' : '#EF4444' }}>
              {fmt(totals.balanceHasu)}
            </div>
            <div className="text-[9px] mt-1 font-medium" style={{ color: '#555' }}>benef. × % hasu</div>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 rounded-t-[20px] p-5 pb-8"
            ...
            <div className="font-black text-[17px] text-white mb-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" value={form.cuenta} onChange={e => setForm(f => ({ ...f, cuenta: e.target.value }))}
                    placeholder="CaixaBank..." list="cuentas-list"
                    className="w-full rounded-xl px-3.5 py-3 text-sm text-white outline-none font-medium" style={INP} />
