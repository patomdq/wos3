...
- **Calculadora JV / Gestor — nueva card en análisis multi-estrategia** (`app/(app)/mercado/page.tsx`)
  ...
  - **Reparto JV visible en la card principal de Mercado (mismo día)**: antes solo se veía dentro de la calculadora. Ahora la card de la lista muestra: badge morado "JV · N" junto a tipología/fuente (arriba de la imagen), y debajo de la tabla Pesimista/Realista/Optimista una fila compacta con cada jugador (nombre · % beneficio · beneficio €) calculada sobre el escenario Realista. Se extrajo la lógica de reparto a la función pura `calcJvReparto()` (antes vivía solo dentro del modal) para reutilizarla en ambos lugares sin duplicar la fórmula. Commit `37a9825`
