...
export function pasaFiltros(p: DeudaPosicion, filtros: DeudaFiltrosState, opts?: { ignorarRiesgo?: boolean }): boolean {
  ...
  const norm = (s?: string | null) => s ? s.trim().toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '') : ''
  if (filtros.provincia !== 'todos' && norm(p.provincia) !== norm(filtros.provincia)) return false
  if (filtros.ciudad !== 'todos' && norm(p.ciudad) !== norm(filtros.ciudad)) return false
